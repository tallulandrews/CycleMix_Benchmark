...
run_CycleMixRegression <- function(obj, phase_column) {
	norm_expr <- GetAssayData(obj, layer="data", assay="RNA")
	out <- regressCyclePartial(norm_expr, obj@meta.data[,phase_column], allow_negative=FALSE, phases=c("G2M", "G1S"), type="norm", method="phase",subsample_cells=5000)
	return(out)
}
